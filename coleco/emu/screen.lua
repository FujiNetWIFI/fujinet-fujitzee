-- screen.lua -- read Fujitzee's text screen out of VRAM and print it.
--
-- The verdict harness for every headless run. Adapted from the ColecoVision
-- bring-up's screen.lua, which decodes a Graphics-1 screen where a name-table
-- byte simply IS its ascii code. Fujitzee runs Graphics II with a BANKED
-- charset -- the same 64 glyphs uploaded three times at three inks, because
-- Mode 2 colours a pattern rather than a cell -- so a code has to be folded
-- back to ascii before any of this reads as text:
--
--   0x00-0x6F  art tiles: dice, the roll tile, the logo, chrome, icons.
--              Shown as the markers below; there is no ascii for them.
--   0x70-0xAF  the font in WHITE            ascii = code - 0x50
--   0xB0-0xEF  the font in CYAN, the accent ascii = code - 0x90; the row is
--              marked, so a verdict can assert where the accent is
--   0xF0-0xFF  digits in LIGHT GREEN -- a potential score, and marked too
--
-- Keeping the fold here rather than in the client is what lets "what is on
-- screen" stay a string comparison instead of a pixel diff.
--
...
-- What an art tile prints as. Not a space: a die and a blank cell must not
-- look the same in a dump, or "the dice are drawn" is unfalsifiable.
...
local function read_screen()
    ...
    -- Fold the banks back to ascii. An accented or green cell marks its row,
    -- so a verdict can assert WHERE the colour is and not merely that the
    -- text is present -- which is the whole point for potential scores.
    local lines = {}
    local marks = {}
    for row = 0, ROWS - 1 do
...
_G.screen_sub = emu.add_machine_frame_notifier(function ()
    ...
    if not QUIET then
        ...
        for i, l in ipairs(lines) do
            -- Row edges: '$' the row carries LIGHT GREEN cells (a potential score),
            -- '#' cyan accent, '|' plain. Colour is the thing most worth
            -- asserting here and it is invisible in the glyphs alone.
            local edge = marks[i]
